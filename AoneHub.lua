-- ──────────────────────────────────────────────────────────────────────
-- AONEHUB - AUTO BUY SEED + GEAR + PROP (TRIPLE ACCORDION)
-- ──────────────────────────────────────────────────────────────────────
...
local function main()
    ...
    loadConfig(); task.spawn(function() while true do task.wait(30); saveConfig() end end)
    ...
    local function getAllSeeds()
        local s, d = pcall(require, ReplicatedStorage.SharedModules.SeedData)
        if not s then return {} end
        local s2, w = pcall(require, ReplicatedStorage.SharedModules.Worlds)
        local items = {}
        for _, seed in ipairs(d) do
            if seed.RestockShop and (not w or not s2 or w.EntryAvailableHere(seed)) then
    ...
    local function getAllGears()
        local s, d = pcall(require, ReplicatedStorage.SharedModules.GearShopData)
        if not s or not d.Data then return {} end
        local s2, w = pcall(require, ReplicatedStorage.SharedModules.Worlds)
        local items = {}
        for _, gear in ipairs(d.Data) do
            if not gear.RobuxOnly and not gear.HideFromShop and (not w or not s2 or w.EntryAvailableHere(gear)) then
    ...
    local function getAllProps()
        local s, d = pcall(require, ReplicatedStorage.SharedModules.CrateData)
        if not s or not d.GetAllCrates then return {} end
        local s2, w = pcall(require, ReplicatedStorage.SharedModules.Worlds)
        local items = {}
        for _, crate in ipairs(d.GetAllCrates()) do
            if crate.RestockChance and (not w or not s2 or w.EntryAvailableHere(crate)) then
    ...
    local function mergeItems(saved, current)
        local merged = {}
        for name, val in pairs(saved or {}) do merged[name] = val end
    ...
    local C = {
        ...
        accordionSeed = Color3.fromRGB(35, 42, 35),   -- Green tint
        accordionGear = Color3.fromRGB(42, 35, 35),   -- Orange tint
        accordionProp = Color3.fromRGB(40, 35, 45),   -- Purple tint
    ...
    -- ==================================================================
    -- SIDEBAR + TABS (same as before, compact)
    -- ==================================================================
    ...
    local function getCarrotStock()
        ...
        local carrot = ns:FindFirstChild("Carrot"); if not carrot then return nil end
        local mf = carrot:FindFirstChild("Main_Frame") or carrot:FindFirstChild("MainFrame"); if not mf then return nil end
    ...
    local function detectOpcode()
        ...
        print("[AutoBuy] 🔍 Detecting from " .. lastScannedOpcode .. "...")
        task.wait(1)
        local stockBefore = getCarrotStock()
        if not stockBefore or stockBefore == 0 then warn("[AutoBuy] ⚠️ Carrot unavailable"); updateOpcodeDisplay(); return end
        for testOpcode = lastScannedOpcode, 200 do
            pcall(function() packetRemote:FireServer(buffer.fromstring(string.char(testOpcode,0,6).."Carrot")) end)
            task.wait(0.8)
            ...
            if stockAfter and stockAfter < stockBefore then
                ...
                config.opcodeDetected = true; config.lastScannedOpcode = lastScannedOpcode
                saveConfig()
                print("[AutoBuy] 🎯 Seed:"..OPCODE_SEED.." Gear:"..OPCODE_GEAR.." Prop:"..OPCODE_PROP)
                updateOpcodeDisplay(); return
            end; lastScannedOpcode = testOpcode; task.wait(0.2)
        end; updateOpcodeDisplay()
    end
    
    local function resetDetection()
        opcodeDetected = false; lastScannedOpcode = 158
        config.opcodeDetected = false; config.lastScannedOpcode = 158; saveConfig(); updateOpcodeDisplay()
    end
    ...
    local function getSecondsUntilNextRestock()
        ...
        local sutr = (mutr*60) - (now%60) + jitter; return sutr <= 0 and sutr + 300 or sutr
    end
    ...
    local function scanAndBuy()
        scanCount += 1; cacheShopElements(); buyHistory = {}; local any = false
    ...
    local function mainLoop()
        while isRunning do local wt = getSecondsUntilNextRestock(); nextScanTime = os.time() + wt; updateUI()
    ...
    local function startMonitoring()
        ...
        if not opcodeDetected then detectOpcode() end
    ...
    scroll.Size = UDim2.new(1, 0, 1, 0); scroll.CanvasSize = UDim2.new(0, 0, 0, 1800)
    ...
    hdr.Size = UDim2.new(1, -20, 0, 26); hdr.Position = UDim2.new(0, 10, 0, y)
    hdr.Text = "🛒  Auto Buy (Seed + Gear + Prop)"; hdr.TextColor3 = C.text
    hdr.Font = Enum.Font.GothamBold; hdr.TextSize = 15
    hdr.TextXAlignment = Enum.TextXAlignment.Left; hdr.BackgroundTransparency = 1; hdr.Parent = scroll
    y += 30
    
    -- Status
    local statusText = Instance.new("TextLabel")
    statusText.Size = UDim2.new(1, -20, 0, 20); statusText.Position = UDim2.new(0, 10, 0, y)
    ...
    opRow.Size = UDim2.new(1, -20, 0, 55); opRow.Position = UDim2.new(0, 10, 0, y)
    opRow.BackgroundTransparency = 1; opRow.Parent = scroll
    
    local opInputSeed, opInputGear, opInputProp = nil, nil, nil
    local opDetectStatus = nil
    
    -- Seed
    local opl1 = Instance.new("TextLabel"); opl1.Size = UDim2.new(0, 38, 0, 16); opl1.Text = "Seed:"; opl1.TextColor3 = Color3.fromRGB(100,200,100); opl1.Font = Enum.Font.Gotham; opl1.TextSize = 10; opl1.BackgroundTransparency = 1; opl1.Parent = opRow
    opInputSeed = Instance.new("TextBox"); opInputSeed.Size = UDim2.new(0, 42, 0, 16); opInputSeed.Position = UDim2.new(0, 40, 0, 0); opInputSeed.Text = tostring(OPCODE_SEED); opInputSeed.TextColor3 = C.text; opInputSeed.Font = Enum.Font.GothamBold; opInputSeed.TextSize = 10; opInputSeed.BackgroundColor3 = C.input; opInputSeed.BorderSizePixel = 0; opInputSeed.Parent = opRow; Instance.new("UICorner", opInputSeed).CornerRadius = UDim.new(0, 3)
    
    -- Gear
    local opl2 = Instance.new("TextLabel"); opl2.Size = UDim2.new(0, 38, 0, 16); opl2.Position = UDim2.new(0, 90, 0, 0); opl2.Text = "Gear:"; opl2.TextColor3 = Color3.fromRGB(255,150,50); opl2.Font = Enum.Font.Gotham; opl2.TextSize = 10; opl2.BackgroundTransparency = 1; opl2.Parent = opRow
    opInputGear = Instance.new("TextBox"); opInputGear.Size = UDim2.new(0, 42, 0, 16); opInputGear.Position = UDim2.new(0, 130, 0, 0); opInputGear.Text = tostring(OPCODE_GEAR); opInputGear.TextColor3 = Color3.fromRGB(255,200,150); opInputGear.Font = Enum.Font.GothamBold; opInputGear.TextSize = 10; opInputGear.BackgroundColor3 = C.input; opInputGear.BorderSizePixel = 0; opInputGear.Editable = false; opInputGear.Parent = opRow; Instance.new("UICorner", opInputGear).CornerRadius = UDim.new(0, 3)
    
    -- Prop
    local opl3 = Instance.new("TextLabel"); opl3.Size = UDim2.new(0, 38, 0, 16); opl3.Position = UDim2.new(0, 180, 0, 0); opl3.Text = "Prop:"; opl3.TextColor3 = Color3.fromRGB(200,100,255); opl3.Font = Enum.Font.Gotham; opl3.TextSize = 10; opl3.BackgroundTransparency = 1; opl3.Parent = opRow
    opInputProp = Instance.new("TextBox"); opInputProp.Size = UDim2.new(0, 42, 0, 16); opInputProp.Position = UDim2.new(0, 220, 0, 0); opInputProp.Text = tostring(OPCODE_PROP); opInputProp.TextColor3 = Color3.fromRGB(220,180,255); opInputProp.Font = Enum.Font.GothamBold; opInputProp.TextSize = 10; opInputProp.BackgroundColor3 = C.input; opInputProp.BorderSizePixel = 0; opInputProp.Editable = false; opInputProp.Parent = opRow; Instance.new("UICorner", opInputProp).CornerRadius = UDim.new(0, 3)
    
    -- Buttons
    local opSetBtn = Instance.new("TextButton"); opSetBtn.Size = UDim2.new(0, 35, 0, 16); opSetBtn.Position = UDim2.new(0, 270, 0, 0); opSetBtn.Text = "Set"; opSetBtn.TextColor3 = C.text; opSetBtn.Font = Enum.Font.GothamSemibold; opSetBtn.TextSize = 9; opSetBtn.BackgroundColor3 = Color3.fromRGB(55,55,65); opSetBtn.BorderSizePixel = 0; opSetBtn.AutoButtonColor = false; opSetBtn.Parent = opRow; Instance.new("UICorner", opSetBtn).CornerRadius = UDim.new(0, 3)
    
    local detectBtn = Instance.new("TextButton"); detectBtn.Size = UDim2.new(0, 45, 0, 16); detectBtn.Position = UDim2.new(0, 310, 0, 0); detectBtn.Text = "Detect"; detectBtn.TextColor3 = C.text; detectBtn.Font = Enum.Font.GothamSemibold; detectBtn.TextSize = 9; detectBtn.BackgroundColor3 = Color3.fromRGB(70,70,80); detectBtn.BorderSizePixel = 0; detectBtn.AutoButtonColor = false; detectBtn.Parent = opRow; Instance.new("UICorner", detectBtn).CornerRadius = UDim.new(0, 3)
    
    local resetBtn = Instance.new("TextButton"); resetBtn.Size = UDim2.new(0, 40, 0, 16); resetBtn.Position = UDim2.new(0, 360, 0, 0); resetBtn.Text = "Reset"; resetBtn.TextColor3 = Color3.fromRGB(255,150,150); resetBtn.Font = Enum.Font.Gotham; resetBtn.TextSize = 9; resetBtn.BackgroundColor3 = Color3.fromRGB(70,50,50); resetBtn.BorderSizePixel = 0; resetBtn.AutoButtonColor = false; resetBtn.Parent = opRow; Instance.new("UICorner", resetBtn).CornerRadius = UDim.new(0, 3)
    
    opDetectStatus = Instance.new("TextLabel"); opDetectStatus.Size = UDim2.new(1, 0, 0, 14); opDetectStatus.Position = UDim2.new(0, 0, 0, 38); opDetectStatus.TextColor3 = Color3.fromRGB(255,200,50); opDetectStatus.Font = Enum.Font.Gotham; opDetectStatus.TextSize = 9; opDetectStatus.TextXAlignment = Enum.TextXAlignment.Left; opDetectStatus.BackgroundTransparency = 1; opDetectStatus.Parent = opRow
    
    local function syncAllOpcodes()
        OPCODE_GEAR = OPCODE_SEED + 4; OPCODE_PROP = OPCODE_SEED + 2
        config.opcodeSeed = OPCODE_SEED; config.opcodeGear = OPCODE_GEAR; config.opcodeProp = OPCODE_PROP
        saveConfig(); updateOpcodeDisplay()
    end
    
    opSetBtn.MouseButton1Click:Connect(function()
        local n = tonumber(opInputSeed.Text); if n and n >= 100 and n <= 200 then OPCODE_SEED = n; syncAllOpcodes() end
    end)
    detectBtn.MouseButton1Click:Connect(function() detectBtn.Text="..."; detectOpcode(); detectBtn.Text="Detect" end)
    resetBtn.MouseButton1Click:Connect(resetDetection)
    ...
    -- ACCORDION BUILDER
    -- ==================================================================
    local function createAccordion(title, items, selectedItems, itemStatus, headerColor, accordionOpenKey, searchKey, scrollRef, yStart)
        local accordionOpen = config[accordionOpenKey]
        
        local container = Instance.new("Frame"); container.Size = UDim2.new(1, -20, 0, 28); container.Position = UDim2.new(0, 10, 0, yStart); container.BackgroundTransparency = 1; container.Parent = scrollRef
        
        local header = Instance.new("TextButton"); header.Size = UDim2.new(1, 0, 0, 28); header.BackgroundColor3 = headerColor; header.BorderSizePixel = 0; header.Text = ""; header.AutoButtonColor = false; header.Parent = container; Instance.new("UICorner", header).CornerRadius = UDim.new(0, 5)
        
        local arrow = Instance.new("TextLabel"); arrow.Size = UDim2.new(0, 20, 1, 0); arrow.Position = UDim2.new(0, 5, 0, 0); arrow.Text = accordionOpen and "▼" or "▶"; arrow.TextColor3 = C.textDim; arrow.Font = Enum.Font.GothamBold; arrow.TextSize = 10; arrow.BackgroundTransparency = 1; arrow.Parent = header
        
        local titleLbl = Instance.new("TextLabel"); titleLbl.Size = UDim2.new(1, -60, 1, 0); titleLbl.Position = UDim2.new(0, 26, 0, 0); titleLbl.Text = title.." ("..#items..")"; titleLbl.TextColor3 = C.text; titleLbl.Font = Enum.Font.GothamSemibold; titleLbl.TextSize = 12; titleLbl.TextXAlignment = Enum.TextXAlignment.Left; titleLbl.BackgroundTransparency = 1; titleLbl.Parent = header
        
        local selectAllBtn = Instance.new("TextButton"); selectAllBtn.Size = UDim2.new(0, 38, 0, 18); selectAllBtn.Position = UDim2.new(1, -43, 0.5, -9); selectAllBtn.Text = "All"; selectAllBtn.TextColor3 = C.text; selectAllBtn.Font = Enum.Font.Gotham; selectAllBtn.TextSize = 9; selectAllBtn.BackgroundColor3 = C.accent; selectAllBtn.BorderSizePixel = 0; selectAllBtn.AutoButtonColor = false; selectAllBtn.Parent = header; Instance.new("UICorner", selectAllBtn).CornerRadius = UDim.new(0, 3)
        
        local body = Instance.new("Frame"); body.Size = UDim2.new(1, 0, 0, 200); body.Position = UDim2.new(0, 0, 0, 30); body.BackgroundColor3 = C.accordionBody; body.BorderSizePixel = 0; body.Visible = accordionOpen; body.Parent = container; Instance.new("UICorner", body).CornerRadius = UDim.new(0, 5)
        
        local searchFrame = Instance.new("Frame"); searchFrame.Size = UDim2.new(1, -8, 0, 24); searchFrame.Position = UDim2.new(0, 4, 0, 4); searchFrame.BackgroundColor3 = C.searchBg; searchFrame.BorderSizePixel = 0; searchFrame.Parent = body; Instance.new("UICorner", searchFrame).CornerRadius = UDim.new(0, 4)
        
        local searchIcon = Instance.new("TextLabel"); searchIcon.Size = UDim2.new(0, 18, 1, 0); searchIcon.Position = UDim2.new(0, 4, 0, 0); searchIcon.Text = "🔍"; searchIcon.TextSize = 9; searchIcon.BackgroundTransparency = 1; searchIcon.Parent = searchFrame
        
        local searchBox = Instance.new("TextBox"); searchBox.Size = UDim2.new(1, -24, 1, 0); searchBox.Position = UDim2.new(0, 22, 0, 0); searchBox.PlaceholderText = "Cari..."; searchBox.PlaceholderColor3 = Color3.fromRGB(100,100,110); searchBox.Text = config[searchKey] or ""; searchBox.TextColor3 = C.text; searchBox.Font = Enum.Font.Gotham; searchBox.TextSize = 10; searchBox.BackgroundTransparency = 1; searchBox.BorderSizePixel = 0; searchBox.Parent = searchFrame
        
        local itemList = Instance.new("ScrollingFrame"); itemList.Size = UDim2.new(1, -8, 1, -32); itemList.Position = UDim2.new(0, 4, 0, 30); itemList.BackgroundTransparency = 1; itemList.BorderSizePixel = 0; itemList.ScrollBarThickness = 3; itemList.Parent = body
        ...
        local function rebuildList()
            ...
            for name, _ in pairs(selectedItems) do if not seen[name] then table.insert(allDisplay, name); seen[name] = true end end
            ...
            itemList.CanvasSize = UDim2.new(0, 0, 0, #filtered * 22 + 2); titleLbl.Text = title.." ("..#filtered..")"
            ...
            for _, itemName in ipairs(filtered) do
                ...
                local row = Instance.new("Frame"); row.Size = UDim2.new(1, 0, 0, 20); row.BackgroundColor3 = isSelected and C.itemRowSelected or C.itemRow; row.BorderSizePixel = 0; row.Parent = itemList; Instance.new("UICorner", row).CornerRadius = UDim.new(0, 3)
                
                local cb = Instance.new("TextButton"); cb.Size = UDim2.new(0, 14, 0, 14); cb.Position = UDim2.new(0, 3, 0.5, -7); cb.Text = isSelected and "✅" or "⬜"; cb.TextSize = 9; cb.BackgroundTransparency = 1; cb.BorderSizePixel = 0; cb.AutoButtonColor = false; cb.Parent = row
                
                local icon = Instance.new("TextLabel"); icon.Size = UDim2.new(0, 14, 1, 0); icon.Position = UDim2.new(0, 18, 0, 0); icon.Text = isAvailable and "🟢" or "🔒"; icon.TextSize = 7; icon.BackgroundTransparency = 1; icon.Parent = row
                
                local lbl = Instance.new("TextLabel"); lbl.Size = UDim2.new(1, -36, 1, 0); lbl.Position = UDim2.new(0, 34, 0, 0); lbl.Text = isAvailable and itemName or itemName.." (off)"; lbl.TextColor3 = isAvailable and (isSelected and Color3.fromRGB(100,255,100) or C.textDim) or Color3.fromRGB(120,120,130); lbl.Font = Enum.Font.Gotham; lbl.TextSize = 10; lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.BackgroundTransparency = 1; lbl.Parent = row
            ...
            local itemH = math.min(#filtered * 22 + 34, 200); body.Size = UDim2.new(1, 0, 0, itemH)
            if accordionOpen then container.Size = UDim2.new(1, -20, 0, itemH + 30) end
        end
        
        searchBox:GetPropertyChangedSignal("Text"):Connect(rebuildList)
        
        selectAllBtn.MouseButton1Click:Connect(function()
            selectAllState = not selectAllState; selectAllBtn.Text = selectAllState and "None" or "All"; selectAllBtn.BackgroundColor3 = selectAllState and C.red or C.accent
            for _, name in ipairs(items) do selectedItems[name] = not selectAllState end; saveConfig(); rebuildList()
        end)
        
        header.MouseButton1Click:Connect(function()
            accordionOpen = not accordionOpen; body.Visible = accordionOpen; arrow.Text = accordionOpen and "▼" or "▶"
            config[accordionOpenKey] = accordionOpen; saveConfig()
            if accordionOpen then container.Size = UDim2.new(1, -20, 0, body.Size.Y.Offset + 30) else container.Size = UDim2.new(1, -20, 0, 28) end
        end)
    ...
    -- Create 3 Accordions
    local seedAcc = createAccordion("🌱 Seeds", ALL_SEEDS, config.selectedSeeds, itemStatusSeed, C.accordionSeed, "accordionSeedOpen", "searchSeed", scroll, y)
    local gearY = y + (config.accordionSeedOpen and 230 or 30) + 8
    local gearAcc = createAccordion("⚙️ Gears", ALL_GEARS, config.selectedGears, itemStatusGear, C.accordionGear, "accordionGearOpen", "searchGear", scroll, gearY)
    local propY = gearY + (config.accordionGearOpen and 230 or 30) + 8
    local propAcc = createAccordion("📦 Props", ALL_PROPS, config.selectedProps, itemStatusProp, C.accordionProp, "accordionPropOpen", "searchProp", scroll, propY)
    
    -- Toggle button
    local toggleY = propY + (config.accordionPropOpen and 230 or 30) + 15
    local toggleBtn = Instance.new("TextButton"); toggleBtn.Size = UDim2.new(1, -20, 0, 38); toggleBtn.Position = UDim2.new(0, 10, 0, toggleY); toggleBtn.Text = "▶  START"; toggleBtn.TextColor3 = C.text; toggleBtn.Font = Enum.Font.GothamBold; toggleBtn.TextSize = 13; toggleBtn.BackgroundColor3 = C.green; toggleBtn.BorderSizePixel = 0; toggleBtn.AutoButtonColor = false; toggleBtn.Parent = scroll; Instance.new("UICorner", toggleBtn).CornerRadius = UDim.new(0, 8)
    ...
    function updateUI()
        ...
        if isRunning then
            ...
            if not isBuying and nextScanTime > 0 then local rem = nextScanTime - os.time(); if rem > 0 then timerText.Text = os.date("%H:%M:%S", nextScanTime) end
            elseif isBuying then timerText.Text = "MEMBORONG..." end
        else statusText.Text = "⏹️  OFF"; statusText.TextColor3 = C.red; toggleBtn.Text = "▶  START"; toggleBtn.BackgroundColor3 = C.green; timerText.Text = "Next scan: --:--:--" end
    ...
    for _, tab in ipairs({"AutoMail", "Ekstra"}) do
        local f = tabFrames[tab]; local ic = Instance.new("TextLabel"); ic.Size = UDim2.new(1, 0, 0, 50); ic.Position = UDim2.new(0, 0, 0.35, -25); ic.Text = tab=="AutoMail" and "📧" or "⚙️"; ic.Font = Enum.Font.Gotham; ic.TextSize = 45; ic.BackgroundTransparency = 1; ic.Parent = f
        local tt = Instance.new("TextLabel"); tt.Size = UDim2.new(1, 0, 0, 28); tt.Position = UDim2.new(0, 0, 0.45, 0); tt.Text = tab=="AutoMail" and "Auto Mail" or "Ekstra"; tt.TextColor3 = C.text; tt.Font = Enum.Font.GothamBold; tt.TextSize = 18; tt.BackgroundTransparency = 1; tt.Parent = f
        local st = Instance.new("TextLabel"); st.Size = UDim2.new(1, 0, 0, 18); st.Position = UDim2.new(0, 0, 0.52, 0); st.Text = "Coming soon..."; st.TextColor3 = C.textDim; st.Font = Enum.Font.Gotham; st.TextSize = 12; st.BackgroundTransparency = 1; st.Parent = f
    end
