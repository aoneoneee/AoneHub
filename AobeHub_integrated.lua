...
-- ==================================================================
-- AODEHUB HATCH MODULE (RAW GITHUB)
-- ==================================================================
local AODEHUB_RAW_URL = "https://raw.githubusercontent.com/aoneoneee/AoneHub/refs/heads/main/AodeHub_module.lua"

task.spawn(function()
    local okHttp, sourceOrError = pcall(function()
        return game:HttpGet(AODEHUB_RAW_URL)
    end)

    if not okHttp then
        warn("[AobeHub] Gagal mengambil AodeHub module:", sourceOrError)
        return
    end

    local chunk, compileError = loadstring(sourceOrError)
    if not chunk then
        warn("[AobeHub] Gagal compile AodeHub module:", compileError)
        return
    end

    local okLoad, AodeModule = pcall(chunk)
    if not okLoad then
        warn("[AobeHub] Gagal menjalankan AodeHub module:", AodeModule)
        return
    end

    if type(AodeModule) ~= "function" then
        warn("[AobeHub] AodeHub module harus return function(HATCH_PARENT)")
        return
    end

    local okMount, mountError = pcall(function()
        AodeModule(tabFrames["Hatch"])
    end)

    if not okMount then
        warn("[AobeHub] Gagal memasang AodeHub ke Hatch:", mountError)
        return
    end

    print("✅ AodeHub berhasil dimuat ke tab Hatch AobeHub")
end)
