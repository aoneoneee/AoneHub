...
-- ==================================================================
-- AODEHUB SPLIT MODULE -> HATCH TAB
-- AodeHub.lua hanya berisi logic + content Hatch.
-- Tidak membuat ScreenGui/tab/sidebar baru.
-- ==================================================================
local function loadAodeHubHatch()
    local source = nil

    -- Prioritas: file lokal agar benar-benar split.
    local paths = {
        "AoneHub/AodeHub.lua",
        "AodeHub.lua",
    }
    ...
    if not source then
        warn("[AobeHub] AodeHub.lua tidak ditemukan.")
        warn("[AobeHub] Letakkan AodeHub.lua di AoneHub/AodeHub.lua atau AodeHub.lua")
        return nil
    end

    local chunk, compileError = loadstring(source)
    if not chunk then
        warn("[AobeHub] Gagal compile AodeHub.lua:", compileError)
        return nil
    end

    local ok, moduleOrError = pcall(chunk)
    if not ok then
        warn("[AobeHub] Gagal load AodeHub.lua:", moduleOrError)
        return nil
    end

    if type(moduleOrError) ~= "function" then
        warn("[AobeHub] AodeHub.lua harus return function(hatchTab)")
        return nil
    end

    local hatchTab = tabFrames["Hatch"]
    if not hatchTab then
        warn("[AobeHub] Tab Hatch tidak ditemukan.")
        return nil
    end

    local okBuild, result = pcall(moduleOrError, hatchTab)
    if not okBuild then
        warn("[AobeHub] Gagal membangun Hatch dari AodeHub:", result)
        return nil
    end

    print("[AobeHub] ✅ AodeHub Hatch berhasil di-load via loadstring")
    return result
end

local AodeHatchModule = loadAodeHubHatch()
