...
-- ==================================================================
-- AODEHUB MODULE (SPLIT LOAD)
-- ==================================================================
-- AodeHub tidak membuat ScreenGui/tab sendiri. Semua UI-nya ditempel
-- langsung ke tab Hatch dan Inventory milik AobeHub.
local AodeHubModule
do
    local ok, result = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/aoneoneee/AoneHub/refs/heads/main/AodeHub_module.lua"))()
    end)

    if ok and type(result) == "table" and type(result.Init) == "function" then
        AodeHubModule = result.Init({
            RootGui = screenGui,
            HatchTab = tabFrames["Hatch"],
            InventoryTab = tabFrames["Inventory"],
            Config = config,
            SaveConfig = saveConfig,
            SendWebhook = sendDiscordWebhook,
        })
        print("[AobeHub] AodeHub module loaded")
    else
        warn("[AobeHub] Gagal load AodeHub_module.lua:", result)
    end
end
