...
do
local hatchConfig = Aode.hatchConfig
local saveHatchConfig = Aode.saveHatchConfig
local getBackpackEggs = Aode.getBackpackEggs
local loadTeamPresets = Aode.loadTeamPresets
local isPetValid = Aode.isPetValid
local getBackpackPets = Aode.getBackpackPets
local splitPetName = Aode.splitPetName
local autoGiftAllFiltered = Aode.autoGiftAllFiltered
local getPlayerPetData = Aode.getPlayerPetData
local countEggsInBackpack = Aode.countEggsInBackpack
local startAutoHatch = Aode.startAutoHatch
local stopAutoHatch = Aode.stopAutoHatch
-- ==================================================================
-- INTEGRATED AODE -> AOBE HATCH / INVENTORY UI
-- Semua UI memakai createSection + createDynamicDropdown AobeHub.
-- ==================================================================
...
local function hatchInput(parent, y, label, key, minValue, maxValue, decimals)
    ...
    box.Text = tostring(hatchConfig[key] or "")
...
local function hatchToggle(parent, y, label, key)
    ...
    button.MouseButton1Click:Connect(function()
        hatchConfig[key] = not hatchConfig[key]
        saveHatchConfig()
        refresh()
    end)
...
local function hatchActionButton(parent, y, textValue, callback, color)
    local button = Instance.new("TextButton")
...
-- HATCH: MAIN
-- --------------------------------------------------------------
local mainSection = createSection(hatchScroll, "⚙️ Main", "Hatch.Main")
mainSection.LayoutOrder = 1

local speedDropdown = createDynamicDropdown(
    mainSection, UDim2.new(0, 10, 0, 32),
    "⚡ Speed — pilih Team Preset",
    mainSection, 250, hatchScroll
)
local hatchDropdown = createDynamicDropdown(
    mainSection, UDim2.new(0, 10, 0, 70),
    "🥚 Hatch — pilih Team Preset",
    mainSection, 250, hatchScroll
)
local sellDropdown = createDynamicDropdown(
    mainSection, UDim2.new(0, 10, 0, 108),
    "💸 Sell — pilih Team Preset",
    mainSection, 250, hatchScroll
)

hatchInput(mainSection, 146, "Jumlah Cycle:", "cycleCount", 1, 100, false)
hatchToggle(mainSection, 176, "💰 Auto Sell", "autoSell")

local hatchStatus = Instance.new("TextLabel")
hatchStatus.Size = UDim2.new(1, -20, 0, 24)
hatchStatus.Position = UDim2.new(0, 10, 0, 205)
hatchStatus.BackgroundTransparency = 1
hatchStatus.TextColor3 = C.textDim
hatchStatus.Font = Enum.Font.Gotham
hatchStatus.TextSize = 8
hatchStatus.TextWrapped = true
hatchStatus.TextXAlignment = Enum.TextXAlignment.Left
hatchStatus.Parent = mainSection

Aode.setStatusCallback(function(textValue)
    if hatchStatus and hatchStatus.Parent then
        hatchStatus.Text = tostring(textValue)
    end
end)

hatchActionButton(mainSection, 232, "▶️ Mulai Auto Hatch", function()
    if Aode.getRunning() then
        stopAutoHatch()
    else
        if next(hatchConfig.selectedEggs) == nil then
            hatchStatus.Text = "⚠️ Pilih minimal 1 egg di section Egg."
            return
        end
        if not hatchConfig.hatchPreset then
            hatchStatus.Text = "⚠️ Pilih preset Hatch terlebih dahulu."
            return
        end
        saveHatchConfig()
        startAutoHatch()
...
-- Populate Aode loadout dropdowns from AobeHub team presets.
local function populateHatchPresetDropdown(dropdown, key, label)
    ...
    for i, name in ipairs(names) do
        ...
        for _, petInfo in ipairs(p.pets or {}) do
            if isPetValid(petInfo.UUID) then valid += 1 end
        end

        local b = Instance.new("TextButton")
        b.Size = UDim2.new(1, -4, 0, dropdown.ItemHeight)
        b.BackgroundColor3 = hatchConfig[key] == name and C.success or Color3.fromRGB(65, 65, 80)
        b.BorderSizePixel = 0
        ...
        b.MouseButton1Click:Connect(function()
            hatchConfig[key] = name
            dropdown.HeaderButton.Text = label .. ": " .. name
            saveHatchConfig()
            dropdown.Close()
            populateHatchPresetDropdown(dropdown, key, label)
        end)
    end
    dropdown.UpdateSize()
end

populateHatchPresetDropdown(speedDropdown, "speedPreset", "⚡ Speed")
populateHatchPresetDropdown(hatchDropdown, "hatchPreset", "🥚 Hatch")
populateHatchPresetDropdown(sellDropdown, "sellPreset", "💸 Sell")

if hatchConfig.speedPreset then speedDropdown.HeaderButton.Text = "⚡ Speed: " .. hatchConfig.speedPreset end
if hatchConfig.hatchPreset then hatchDropdown.HeaderButton.Text = "🥚 Hatch: " .. hatchConfig.hatchPreset end
if hatchConfig.sellPreset then sellDropdown.HeaderButton.Text = "💸 Sell: " .. hatchConfig.sellPreset end

-- --------------------------------------------------------------
-- HATCH: EGG
-- --------------------------------------------------------------
local eggSection = createSection(hatchScroll, "📦 Egg", "Hatch.Egg")
eggSection.LayoutOrder = 2
eggSection.Size = UDim2.new(1, -10, 0, 310)

local eggRefresh = hatchActionButton(eggSection, 32, "🔄 Refresh Egg List", function() end)

local eggListFrame = Instance.new("ScrollingFrame")
eggListFrame.Size = UDim2.new(1, -20, 0, 235)
eggListFrame.Position = UDim2.new(0, 10, 0, 66)
...
local function refreshIntegratedEggList()
    ...
    for _, eggInfo in ipairs(getBackpackEggs()) do
        eggCounts[eggInfo.name] = (eggCounts[eggInfo.name] or 0) + eggInfo.count
    end

    local names = {}
    for name in pairs(eggCounts) do table.insert(names, name) end
    table.sort(names)

    for _, name in ipairs(names) do
        local selected = hatchConfig.selectedEggs[name] == true
        local b = Instance.new("TextButton")
        b.Size = UDim2.new(1, 0, 0, 22)
        b.BackgroundColor3 = selected and C.success or Color3.fromRGB(65, 65, 80)
        b.BorderSizePixel = 0
        b.Font = Enum.Font.Gotham
        b.TextSize = 8
        b.TextColor3 = C.text
        b.Text = string.format("%s %s (x%d)", selected and "✓" or "□", name, eggCounts[name])
        b.TextXAlignment = Enum.TextXAlignment.Left
        b.Parent = eggListFrame
        Instance.new("UICorner", b).CornerRadius = UDim.new(0, 3)
        local pad = Instance.new("UIPadding")
        ...
        b.MouseButton1Click:Connect(function()
            hatchConfig.selectedEggs[name] = not selected and true or nil
            saveHatchConfig()
            refreshIntegratedEggList()
        end)
    end
    ...
    if #names == 0 then
        ...
        info.Text = "Tidak ada egg di backpack."
        info.TextColor3 = C.textDim
        info.Font = Enum.Font.Gotham
        info.TextSize = 8
        info.Parent = eggListFrame
    end
end

eggRefresh.MouseButton1Click:Connect(refreshIntegratedEggList)
refreshIntegratedEggList()

-- --------------------------------------------------------------
-- HATCH: DELAY
-- --------------------------------------------------------------
local delaySection = createSection(hatchScroll, "⏱️ Delay", "Hatch.Delay")
delaySection.LayoutOrder = 3
delaySection.Size = UDim2.new(1, -10, 0, 330)

hatchInput(delaySection, 32, "Loadout Swap:", "delayAfterLoadout", 0, 60, true)
hatchInput(delaySection, 62, "Setelah Hatch All:", "delayAfterHatchAll", 0, 60, true)
hatchInput(delaySection, 92, "Antar Hatch:", "hatchDelay", 0, 60, true)
hatchInput(delaySection, 122, "Place Egg:", "placeDelay", 0, 60, true)
hatchInput(delaySection, 152, "Scan Settle:", "scanSettleDelay", 0, 60, true)
hatchInput(delaySection, 182, "Setelah Speed:", "delayAfterSpeed", 0, 60, true)
hatchInput(delaySection, 212, "Setelah Hatch:", "delayAfterHatch", 0, 60, true)
hatchInput(delaySection, 242, "Setelah Sell:", "delayAfterSell", 0, 60, true)
hatchInput(delaySection, 272, "Sebelum Sell:", "delayBeforeSell", 0, 60, true)
hatchInput(delaySection, 302, "Gift → Hatch Sync:", "delayAfterGiftSync", 0, 120, true)

-- --------------------------------------------------------------
-- HATCH: UNWANTED
-- --------------------------------------------------------------
local unwantedSection = createSection(hatchScroll, "🎯 Unwanted", "Hatch.Unwanted")
unwantedSection.LayoutOrder = 4
unwantedSection.Size = UDim2.new(1, -10, 0, 410)

hatchInput(unwantedSection, 32, "Max Weight (KG):", "maxWeight", 0, 1000, true)
hatchInput(unwantedSection, 62, "Max Level:", "maxLevel", 0, 10000, false)

local unwantedInfo = Instance.new("TextLabel")
unwantedInfo.Size = UDim2.new(1, -20, 0, 22)
unwantedInfo.Position = UDim2.new(0, 10, 0, 92)
unwantedInfo.BackgroundTransparency = 1
unwantedInfo.Text = "Pet diproses jika termasuk unwanted + weight ≤ max + level ≤ max."
unwantedInfo.TextColor3 = C.textDim
unwantedInfo.Font = Enum.Font.Gotham
unwantedInfo.TextSize = 7
unwantedInfo.TextWrapped = true
unwantedInfo.TextXAlignment = Enum.TextXAlignment.Left
unwantedInfo.Parent = unwantedSection

local unwantedList = Instance.new("ScrollingFrame")
unwantedList.Size = UDim2.new(1, -20, 0, 275)
unwantedList.Position = UDim2.new(0, 10, 0, 125)
unwantedList.BackgroundTransparency = 1
unwantedList.BorderSizePixel = 0
unwantedList.ScrollBarThickness = 3
unwantedList.AutomaticCanvasSize = Enum.AutomaticSize.Y
unwantedList.CanvasSize = UDim2.new(0, 0, 0, 0)
unwantedList.Parent = unwantedSection

local unwantedLayout = Instance.new("UIListLayout")
unwantedLayout.Padding = UDim.new(0, 3)
unwantedLayout.Parent = unwantedList

local function refreshIntegratedUnwantedList()
    for _, child in ipairs(unwantedList:GetChildren()) do
        if child:IsA("TextButton") then child:Destroy() end
    end
    ...
    local names = {}
    for name in pairs(types) do table.insert(names, name) end
    table.sort(names)

    for _, name in ipairs(names) do
        local selected = hatchConfig.unwantedPetTypes[name] == true
        local b = Instance.new("TextButton")
        b.Size = UDim2.new(1, 0, 0, 22)
        b.BackgroundColor3 = selected and C.danger or Color3.fromRGB(65, 65, 80)
        b.BorderSizePixel = 0
        b.Font = Enum.Font.Gotham
        b.TextSize = 8
        b.TextColor3 = C.text
        b.Text = (selected and "✓ " or "□ ") .. name
        b.TextXAlignment = Enum.TextXAlignment.Left
        b.Parent = unwantedList
        Instance.new("UICorner", b).CornerRadius = UDim.new(0, 3)
        local pad = Instance.new("UIPadding")
        pad.PaddingLeft = UDim.new(0, 7)
        pad.Parent = b
        ...
        b.MouseButton1Click:Connect(function()
            hatchConfig.unwantedPetTypes[name] = not selected and true or nil
            saveHatchConfig()
            refreshIntegratedUnwantedList()
            refreshUnwantedMutationsCache()
        end)
    end
end
...
-- INVENTORY TAB
-- --------------------------------------------------------------
local inventoryTab = tabFrames["Inventory"]
...
inventoryScroll.Parent = inventoryTab
...
-- Auto Accept Gift
local acceptSection = createSection(inventoryScroll, "🎁 Auto Accept Pet Gift", "Inventory.AutoAcceptGift")
acceptSection.LayoutOrder = 1
acceptSection.Size = UDim2.new(1, -10, 0, 175)
hatchToggle(acceptSection, 32, "Auto Accept Gift", "giftEnabled")
hatchToggle(acceptSection, 62, "Sembunyikan UI Gift", "giftHideUI")
hatchInput(acceptSection, 92, "Delay antar gift:", "giftDelayBetween", 0, 60, true)

local acceptStatus = Instance.new("TextLabel")
acceptStatus.Size = UDim2.new(1, -20, 0, 35)
acceptStatus.Position = UDim2.new(0, 10, 0, 125)
acceptStatus.BackgroundTransparency = 1
acceptStatus.Text = "Auto accept aktif saat fase tunggu egg timer."
acceptStatus.TextColor3 = C.textDim
acceptStatus.TextSize = 8
acceptStatus.Font = Enum.Font.Gotham
acceptStatus.TextWrapped = true
acceptStatus.TextXAlignment = Enum.TextXAlignment.Left
acceptStatus.Parent = acceptSection

-- Auto Gift Pet
local autoGiftSection = createSection(inventoryScroll, "🎁 Auto Gift Pet", "Inventory.AutoGiftPet")
autoGiftSection.LayoutOrder = 2
autoGiftSection.Size = UDim2.new(1, -10, 0, 355)

local targetDropdown = createDynamicDropdown(
    autoGiftSection, UDim2.new(0, 10, 0, 32),
    "👤 Pilih Target Player",
    autoGiftSection, 355, inventoryScroll
)

hatchInput(autoGiftSection, 70, "Min Weight (KG):", "giftPetMinWeight", 0, 1000, true)
hatchInput(autoGiftSection, 100, "Max Weight (KG):", "giftPetMaxWeight", 0, 1000, true)
hatchInput(autoGiftSection, 130, "Min Level:", "giftPetMinLevel", 0, 10000, false)
hatchInput(autoGiftSection, 160, "Max Level:", "giftPetMaxLevel", 0, 10000, false)

local giftRunningLabel = Instance.new("TextLabel")
giftRunningLabel.Size = UDim2.new(1, -20, 0, 24)
giftRunningLabel.Position = UDim2.new(0, 10, 0, 194)
giftRunningLabel.BackgroundTransparency = 1
giftRunningLabel.TextColor3 = C.textDim
giftRunningLabel.Font = Enum.Font.Gotham
giftRunningLabel.TextSize = 8
giftRunningLabel.TextXAlignment = Enum.TextXAlignment.Left
giftRunningLabel.Parent = autoGiftSection

hatchActionButton(autoGiftSection, 225, "🎁 Mulai / Stop Auto Gift", function()
    if Aode.getGiftRunning() then
        Aode.setGiftRunning(false)
        Aode.setGiftUserStarted(false)
        giftRunningLabel.Text = "⏹️ Auto Gift dihentikan."
    else
        if not hatchConfig.giftPetTarget then
            giftRunningLabel.Text = "⚠️ Pilih target player dulu."
            return
        end
        Aode.setGiftUserStarted(true)
        Aode.setGiftRunning(true)
        saveHatchConfig()
        giftRunningLabel.Text = "▶️ Auto Gift berjalan..."
        task.spawn(function()
            autoGiftAllFiltered()
            if Aode.getGiftRunning() then
                Aode.setGiftRunning(false)
                giftRunningLabel.Text = "✅ Auto Gift selesai."
            end
        end)
    end
end, C.accent)

local function populateTargetPlayerDropdown()
    for _, child in ipairs(targetDropdown.ListScroll:GetChildren()) do
        if child:IsA("TextButton") then child:Destroy() end
    end
    ...
    for i, name in ipairs(names) do
        local b = Instance.new("TextButton")
        b.Size = UDim2.new(1, -4, 0, targetDropdown.ItemHeight)
        b.BackgroundColor3 = hatchConfig.giftPetTarget == name and C.success or Color3.fromRGB(65,65,80)
        b.BorderSizePixel = 0
        b.Font = Enum.Font.Gotham
        b.TextSize = 8
        b.TextColor3 = C.text
        ...
        b.LayoutOrder = i
        b.ZIndex = 12
        b.Parent = targetDropdown.ListScroll
        Instance.new("UICorner", b).CornerRadius = UDim.new(0,3)
        local pad = Instance.new("UIPadding")
        pad.PaddingLeft = UDim.new(0,6)
        pad.Parent = b
        ...
        b.MouseButton1Click:Connect(function()
            hatchConfig.giftPetTarget = name
            targetDropdown.HeaderButton.Text = "👤 " .. name
            saveHatchConfig()
            targetDropdown.Close()
        end)
    end
    targetDropdown.UpdateSize()
end
populateTargetPlayerDropdown()
if hatchConfig.giftPetTarget then
    targetDropdown.HeaderButton.Text = "👤 " .. tostring(hatchConfig.giftPetTarget)
end

-- Pet type filter for Auto Gift
local giftPetSection = createSection(inventoryScroll, "📋 Pet untuk Gift", "Inventory.GiftPetFilter")
giftPetSection.LayoutOrder = 3
giftPetSection.Size = UDim2.new(1, -10, 0, 310)

local giftPetList = Instance.new("ScrollingFrame")
giftPetList.Size = UDim2.new(1, -20, 0, 255)
giftPetList.Position = UDim2.new(0, 10, 0, 32)
giftPetList.BackgroundTransparency = 1
giftPetList.BorderSizePixel = 0
giftPetList.ScrollBarThickness = 3
giftPetList.AutomaticCanvasSize = Enum.AutomaticSize.Y
giftPetList.CanvasSize = UDim2.new(0,0,0,0)
giftPetList.Parent = giftPetSection

local giftPetLayout = Instance.new("UIListLayout")
giftPetLayout.Padding = UDim.new(0,3)
giftPetLayout.Parent = giftPetList

local function refreshGiftPetTypes()
    for _, child in ipairs(giftPetList:GetChildren()) do
        if child:IsA("TextButton") then child:Destroy() end
    end
    ...
    for _, name in ipairs(names) do
        local selected = hatchConfig.giftPetSelectedTypes[name] == true
        local b = Instance.new("TextButton")
        b.Size = UDim2.new(1,0,0,22)
        b.BackgroundColor3 = selected and C.success or Color3.fromRGB(65,65,80)
        b.BorderSizePixel = 0
        ...
        b.MouseButton1Click:Connect(function()
            if selected then
                hatchConfig.giftPetSelectedTypes[name] = nil
            else
                hatchConfig.giftPetSelectedTypes[name] = true
            end
            saveHatchConfig()
            refreshGiftPetTypes()
        end)
...
-- Inventory info
local invInfoSection = createSection(inventoryScroll, "📊 Inventory Info", "Inventory.Info")
invInfoSection.LayoutOrder = 4
invInfoSection.Size = UDim2.new(1, -10, 0, 90)

local invInfo = Instance.new("TextLabel")
invInfo.Size = UDim2.new(1, -20, 1, -38)
invInfo.Position = UDim2.new(0,10,0,32)
invInfo.BackgroundTransparency = 1
invInfo.TextColor3 = C.textDim
invInfo.Font = Enum.Font.Gotham
invInfo.TextSize = 8
invInfo.TextWrapped = true
invInfo.TextXAlignment = Enum.TextXAlignment.Left
invInfo.Parent = invInfoSection

local function refreshIntegratedInventoryInfo()
    local petsData = getPlayerPetData()
    if not petsData or not petsData.PetInventory then
        invInfo.Text = "Inventory tidak tersedia."
        return
    end

    local current = 0
    for _ in pairs(petsData.PetInventory.Data or {}) do current += 1 end
    local max = petsData.MutableStats and petsData.MutableStats.MaxPetsInInventory or 0
    local eggs = countEggsInBackpack()

    invInfo.Text = string.format(
        "🐾 Pet: %d / %d\n🥚 Egg terpilih: %d",
        current, max, eggs
    )
end

hatchActionButton(invInfoSection, 2, "🔄 Refresh Inventory", function()
    refreshGiftPetTypes()
    refreshIntegratedUnwantedList()
    refreshIntegratedInventoryInfo()
    populateTargetPlayerDropdown()
end, C.accent)

refreshIntegratedInventoryInfo()

-- Keep dynamic inventory/egg lists useful after opening the UI.
task.spawn(function()
    while not Aode.getGuiDestroyed() do
        task.wait(5)
        if hatchTab.Visible then
            refreshIntegratedEggList()
            refreshGiftPetTypes()
            refreshIntegratedUnwantedList()
            refreshIntegratedInventoryInfo()
