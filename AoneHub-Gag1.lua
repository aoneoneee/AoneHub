...
AUI.hatchInput(mainSection, 146, "Jumlah Cycle:", "cycleCount", 1, 100, false)
AUI.hatchToggle(mainSection, 176, "💰 Auto Sell", "autoSell")
...
AUI.hatchActionButton(mainSection, 232, "▶️ Mulai Auto Hatch", function()
    if Aode.getRunning() then
        AUI.stopAutoHatch()
    else
        if next(AUI.hatchConfig.selectedEggs) == nil then
            hatchStatus.Text = "⚠️ Pilih minimal 1 egg di section Egg."
            return
        end
        if not AUI.hatchConfig.hatchPreset then
...
AUI.populateHatchPresetDropdown(speedDropdown, "speedPreset", "⚡ Speed")
AUI.populateHatchPresetDropdown(hatchDropdown, "hatchPreset", "🥚 Hatch")
AUI.populateHatchPresetDropdown(sellDropdown, "sellPreset", "💸 Sell")

if AUI.hatchConfig.speedPreset then speedDropdown.HeaderButton.Text = "⚡ Speed: " .. AUI.hatchConfig.speedPreset end
if AUI.hatchConfig.hatchPreset then hatchDropdown.HeaderButton.Text = "🥚 Hatch: " .. AUI.hatchConfig.hatchPreset end
if AUI.hatchConfig.sellPreset then sellDropdown.HeaderButton.Text = "💸 Sell: " .. AUI.hatchConfig.sellPreset end
...
local eggRefresh = AUI.hatchActionButton(eggSection, 32, "🔄 Refresh Egg List", function() end)
...
function AUI.refreshIntegratedEggList()
    ...
    local eggCounts = {}
    for _, eggInfo in ipairs(AUI.getBackpackEggs()) do
...
AUI.hatchInput(delaySection, 32, "Loadout Swap:", "delayAfterLoadout", 0, 60, true)
AUI.hatchInput(delaySection, 62, "Setelah Hatch All:", "delayAfterHatchAll", 0, 60, true)
AUI.hatchInput(delaySection, 92, "Antar Hatch:", "hatchDelay", 0, 60, true)
AUI.hatchInput(delaySection, 122, "Place Egg:", "placeDelay", 0, 60, true)
AUI.hatchInput(delaySection, 152, "Scan Settle:", "scanSettleDelay", 0, 60, true)
AUI.hatchInput(delaySection, 182, "Setelah Speed:", "delayAfterSpeed", 0, 60, true)
AUI.hatchInput(delaySection, 212, "Setelah Hatch:", "delayAfterHatch", 0, 60, true)
AUI.hatchInput(delaySection, 242, "Setelah Sell:", "delayAfterSell", 0, 60, true)
AUI.hatchInput(delaySection, 272, "Sebelum Sell:", "delayBeforeSell", 0, 60, true)
AUI.hatchInput(delaySection, 302, "Gift → Hatch Sync:", "delayAfterGiftSync", 0, 120, true)
...
AUI.hatchInput(unwantedSection, 32, "Max Weight (KG):", "maxWeight", 0, 1000, true)
AUI.hatchInput(unwantedSection, 62, "Max Level:", "maxLevel", 0, 10000, false)
...
function AUI.refreshIntegratedUnwantedList()
    ...
    local types = {}
    for _, pet in ipairs(AUI.getBackpackPets()) do
        local baseName = AUI.splitPetName(pet.name)
        types[baseName] = true
    end
    ...
    for _, name in ipairs(names) do
        ...
        b.MouseButton1Click:Connect(function()
            AUI.hatchConfig.unwantedPetTypes[name] = not selected and true or nil
            AUI.saveHatchConfig()
...
AUI.hatchToggle(acceptSection, 32, "Auto Accept Gift", "giftEnabled")
AUI.hatchToggle(acceptSection, 62, "Sembunyikan UI Gift", "giftHideUI")
AUI.hatchInput(acceptSection, 92, "Delay antar gift:", "giftDelayBetween", 0, 60, true)
...
AUI.hatchInput(autoGiftSection, 70, "Min Weight (KG):", "giftPetMinWeight", 0, 1000, true)
AUI.hatchInput(autoGiftSection, 100, "Max Weight (KG):", "giftPetMaxWeight", 0, 1000, true)
AUI.hatchInput(autoGiftSection, 130, "Min Level:", "giftPetMinLevel", 0, 10000, false)
AUI.hatchInput(autoGiftSection, 160, "Max Level:", "giftPetMaxLevel", 0, 10000, false)
...
AUI.hatchActionButton(autoGiftSection, 225, "🎁 Mulai / Stop Auto Gift", function()
    if Aode.getGiftRunning() then
        ...
        if not AUI.hatchConfig.giftPetTarget then
...
function AUI.populateTargetPlayerDropdown()
    ...
    for i, name in ipairs(names) do
        ...
        b.BackgroundColor3 = AUI.hatchConfig.giftPetTarget == name and C.success or Color3.fromRGB(65,65,80)
        b.BorderSizePixel = 0
        b.Font = Enum.Font.Gotham
        b.TextSize = 8
        b.TextColor3 = C.text
        ...
        b.MouseButton1Click:Connect(function()
            AUI.hatchConfig.giftPetTarget = name
...
if AUI.hatchConfig.giftPetTarget then
    targetDropdown.HeaderButton.Text = "👤 " .. tostring(AUI.hatchConfig.giftPetTarget)
end
...
function AUI.refreshGiftPetTypes()
    ...
    for _, name in ipairs(names) do
        ...
        b.MouseButton1Click:Connect(function()
            if selected then
                AUI.hatchConfig.giftPetSelectedTypes[name] = nil
            else
                AUI.hatchConfig.giftPetSelectedTypes[name] = true
            end
            AUI.saveHatchConfig()
...
function AUI.refreshIntegratedInventoryInfo()
    local petsData = AUI.getPlayerPetData()
    ...
    local eggs = AUI.countEggsInBackpack()
...
AUI.hatchActionButton(invInfoSection, 2, "🔄 Refresh Inventory", function()
    AUI.refreshGiftPetTypes()
    AUI.refreshIntegratedUnwantedList()
