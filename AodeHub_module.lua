...
function Module.Init(context)
    ...
    moduleContext = ctx
    if not ctx.HatchTab or not ctx.InventoryTab then
        warn("[AodeHub] HatchTab / InventoryTab tidak tersedia")
        return false
    end

    guiDestroyed = false
    if ctx.RootGui then
        ctx.RootGui.Destroying:Connect(function()
            guiDestroyed = true
            isRunning = false
            isGiftPetRunning = false
            saveConfig()
    ...
    if ctx.Config then
        CONFIG.webhookEnabled = ctx.Config.discordWebhookEnabled == true
        CONFIG.webhookUrl = tostring(ctx.Config.discordWebhook or "")
    end
...
local webhookToggle = makeToggle(webhookSection, 26, "🔔 Webhook: ON", "🔔 Webhook: OFF", "webhookEnabled")
if ctx.Config then
    webhookToggle.MouseButton1Click:Connect(function()
        ctx.Config.discordWebhookEnabled = CONFIG.webhookEnabled
...
webhookInput.Text = ctx.Config and tostring(ctx.Config.discordWebhook or "") or (CONFIG.webhookUrl or "")
...
webhookInput.FocusLost:Connect(function()
    ...
    if ctx.Config then
        ctx.Config.discordWebhook = CONFIG.webhookUrl
        if ctx.SaveConfig then ctx.SaveConfig() end
    end
    saveConfig()
end)
...
-- TAB: DELAY
local delayTab = ctx.HatchTab
local delayScroll = mainScroll

-- Shared Hatch layout: delayLayout omitted; Aode sections use mainScroll

local delaySection = makeSection(delayScroll, "⏱️ Delay Settings", 200, 6)
...
-- TAB: UNWANTED
local unwantedTab = ctx.HatchTab
local unwantedScrollOuter = mainScroll

-- Shared Hatch layout: unwantedOuterLayout omitted; Aode sections use mainScroll

local settingsSection = makeSection(unwantedScrollOuter, "⚙️ Filter Settings", 100, 7)
...
local petListSection = makeSection(unwantedScrollOuter, "🎯 Pilih Pet Unwanted", 300, 8)
...
task.spawn(function()
    ...
    return {
        ...
        RefreshEggList = refreshEggList,
        RefreshUnwantedList = refreshUnwantedList,
        RefreshGiftPetList = refreshGiftPetList,
    }
end
