...
-- ==================================================================
-- SPLIT MODULE GUI: HATCH CONTENT FOR AOBEHUB
-- ==================================================================
return function(hatchTab)
    guiDestroyed = false

    local hatchScroll = Instance.new("ScrollingFrame")
    hatchScroll.Name = "AodeHatchContent"
    ...
    hatchTab.AncestryChanged:Connect(function(_, parent)
        if not parent then
            guiDestroyed = true
            isRunning = false
            isGiftPetRunning = false
...
-- TAB: MAIN
local mainScroll = hatchScroll

local function makeGroupHeader(parent, text, order)
...
makeGroupHeader(hatchScroll, "⚙️ MAIN", -100)
makeGroupHeader(hatchScroll, "⚙️ MAIN")
local function makeGroupHeader(parent, text, order)
    local h = Instance.new("TextLabel")
    h.Size = UDim2.new(1, -10, 0, 22)
    h.BackgroundTransparency = 1
...
-- GROUP: EGG
makeGroupHeader(hatchScroll, "📦 EGG")
local eggScrollOuter = hatchScroll
...
-- GROUP: DELAY
makeGroupHeader(hatchScroll, "⏱️ DELAY")
local delayScroll = hatchScroll
...
-- GROUP: UNWANTED
makeGroupHeader(hatchScroll, "🎯 UNWANTED")
local unwantedScrollOuter = hatchScroll
...
searchBox:GetPropertyChangedSignal("Text"):Connect(refreshUnwantedList)
    ...
    return {
        Stop = stopAutoHatch,
        RefreshEggs = refreshEggList,
        RefreshUnwanted = refreshUnwantedList,
