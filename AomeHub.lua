...
Instance.new("UICorner", minimizedCircle).CornerRadius = UDim.new(0, 15) -- Rounded square
...
do
    ...
    weightScroll.CanvasSize = UDim2.new(0, 0, 0, 800)
...
autoSprinklerToggle.TextSize = 8
autoSprinklerToggle.BorderSizePixel = 0
autoSprinklerToggle.AutoButtonColor = false
autoSprinklerToggle.Parent = weightScroll
Instance.new("UICorner", autoSprinklerToggle).CornerRadius = UDim.new(0, 4)

autoSprinklerToggle.MouseButton1Click:Connect(function()
    config.weightAutoSprinklerBeforeScan = not config.weightAutoSprinklerBeforeScan
    saveConfig()
    
    if config.weightAutoSprinklerBeforeScan then
        autoSprinklerToggle.BackgroundColor3 = Color3.fromRGB(0, 160, 100)
        autoSprinklerToggle.Text = "💦 Auto Sprinkler Sebelum Scan: ON"
    else
...
sprinklerCooldownInput.FocusLost:Connect(function()
    ...
    autoWaterToggle.LayoutOrder = 15  -- Sesuaikan dengan layout order yang ada
    ...
    autoWaterToggle.Font = Enum.Font.GothamBold
    autoWaterToggle.TextSize = 8
    autoWaterToggle.BorderSizePixel = 0
    autoWaterToggle.AutoButtonColor = false
    autoWaterToggle.Parent = weightScroll
    Instance.new("UICorner", autoWaterToggle).CornerRadius = UDim.new(0, 4)

    autoWaterToggle.MouseButton1Click:Connect(function()
        config.weightAutoWaterAfterShovel = not config.weightAutoWaterAfterShovel
        saveConfig()
    
        if config.weightAutoWaterAfterShovel then
            autoWaterToggle.BackgroundColor3 = Color3.fromRGB(0, 160, 200)
            autoWaterToggle.Text = "💧 Auto Water Setelah Shovel: ON"
        else
            autoWaterToggle.BackgroundColor3 = Color3.fromRGB(40, 40, 45)
            autoWaterToggle.Text = "💧 Auto Water Setelah Shovel: OFF"
        end
    end)
...
local function doAutoSprinklerOnce()
    ...
    if lastSprinklerPlaceTime > 0 then
        local elapsed = currentTime - lastSprinklerPlaceTime
        if elapsed < cooldown then
            local remaining = cooldown - elapsed
            weightStatus.Text = "💦 Sprinkler cooldown: " .. remaining .. "s"
            weightStatus.TextColor3 = C.yellow
            return false
        end
    end
    ...
    if not sprinklerName then
        weightStatus.Text = "💦 Sprinkler skipped (pilih sprinkler di Tools)"
        weightStatus.TextColor3 = C.yellow
        return false
    end
    
    -- Cek stock sprinkler
    local function getSprinklerStock(sprinklerName)
        local total = 0
        local bp = player:FindFirstChild("Backpack")
        if bp then
            local tool = bp:FindFirstChild(sprinklerName)
            if tool then
                local count = tool:GetAttribute("Count")
                if count and type(count) == "number" then total = total + count else total = total + 1 end
            end
        ...
        local char = player.Character
        if char then
            local tool = char:FindFirstChild(sprinklerName)
            if tool then
                local count = tool:GetAttribute("Count")
                if count and type(count) == "number" then total = total + count else total = total + 1 end
            end
    ...
    local stock = getSprinklerStock(sprinklerName)
    if stock <= 0 then
        weightStatus.Text = "💦 Sprinkler skipped (stock habis)"
        weightStatus.TextColor3 = C.yellow
        return false
    end
    ...
    if not targetPos then
        weightStatus.Text = "💦 Sprinkler gagal (tidak ada target)"
        weightStatus.TextColor3 = C.red
        return false
    end
    
    -- Equip sprinkler
    local function getSprinklerTool(sprinklerName)
        local char = player.Character
        if char then
            local tool = char:FindFirstChild(sprinklerName)
            if tool then return tool end
        ...
        if bp then
            for _, item in ipairs(bp:GetChildren()) do
                if item:IsA("Tool") and item.Name == sprinklerName then
                    local h = char and char:FindFirstChildOfClass("Humanoid")
                    if h then h:EquipTool(item); task.wait(0.3) end
                    return char and char:FindFirstChild(sprinklerName)
                end
            end
        end
        return nil
    ...
    if not tool then
        weightStatus.Text = "💦 Sprinkler gagal (tidak bisa equip)"
        weightStatus.TextColor3 = C.red
        return false
    end
    ...
    if not plotId then
        weightStatus.Text = "💦 Sprinkler gagal (tidak ada plot)"
        weightStatus.TextColor3 = C.red
        return false
    end
...
-- ==================================================================
do
    local parentTools = tabFrames["Tools"]
    
    -- Load networking
    local networkingTools = safeRequire(ReplicatedStorage.SharedModules.Networking)
    if not networkingTools then
        print("[AoneHub] ❌ Networking not found for Tools!")
        return
    end
    ...
    pcall(function()
        local mod = safeRequire(ReplicatedStorage.SharedModules.SprinklerData)
        if mod then
            for _, d in ipairs(mod) do
                if d.SprinklerName then table.insert(sprinklerNames, d.SprinklerName) end
            end
        end
        table.sort(sprinklerNames)
    end)
    
    local wateringCanNames = {
    ...
    local selectedWateringCan = config.toolsWateringCan or ""
    local isPlacing = false
    local autoRunning = config.toolsIsAutoSprinkler or false
    local isAutoWateringRunning = config.toolsIsAutoWatering or false
    local sessionTimer = 121
    local sessionActive = false
    local sessionStartTime = 0
    local sessionPlacedCount = 0
    local sessionPositions = {}
    local placedCount = 0
    local wateringInterval = config.toolsWateringInterval or 30
    local sprinklerBatchDone = false
    
    -- Trowel state
    local selectedTrowelPlants = config.toolsTrowelSelected or {}
    local trowelTargetPosition = nil
    local isTrowelRunning = config.toolsIsTrowelRunning or false
    local trowelInterval = config.toolsTrowelInterval or 1.0
    local trowelSearchText = ""
    local trowelAccordionOpen = true
    ...
    local function getSprinklerTool(sprinklerName)
        local char = player.Character
        if char then local tool = char:FindFirstChild(sprinklerName); if tool then return tool end end
        local bp = player:FindFirstChild("Backpack")
        if bp then
            for _, item in ipairs(bp:GetChildren()) do
                if item:IsA("Tool") and item.Name == sprinklerName then
    ...
    local function getWateringCanTool(waterCanName)
        local char = player.Character
        if char then local tool = char:FindFirstChild(waterCanName); if tool then return tool end end
        local bp = player:FindFirstChild("Backpack")
        if bp then
            for _, item in ipairs(bp:GetChildren()) do
                if item:IsA("Tool") and item.Name == waterCanName then
    ...
    local function getTrowelTool()
        ...
        if char then
            for _, item in ipairs(char:GetChildren()) do
                if item:IsA("Tool") and item:GetAttribute("Trowel") then return item end
            end
        end
        local bp = player:FindFirstChild("Backpack")
        if bp then
            for _, item in ipairs(bp:GetChildren()) do
                if item:IsA("Tool") and item:GetAttribute("Trowel") then
                    local h = char and char:FindFirstChildOfClass("Humanoid")
                    if h then h:EquipTool(item); task.wait(0.5) end
                    return char and char:FindFirstChild(item.Name) or item
                end
            end
        end
        return nil
    ...
    local function scanPlants()
        ...
        for _, plantModel in ipairs(plantsFolder:GetChildren()) do
            if plantModel:IsA("Model") then
                local seedName = plantModel:GetAttribute("SeedName")
                if seedName and seedName ~= "" and not seenNames[seedName] then
                    seenNames[seedName] = true
                    table.insert(plants, {name = seedName, model = plantModel})
                end
            end
        end
    ...
    local function getOffsetPosition(position, offsetIndex)
        local offsets = {
            Vector3.new(0, 0, 0),
        ...
        local idx = ((offsetIndex or 0) % #offsets) + 1
        return position + offsets[idx]
    end
    ...
    local function placeSprinklerAt(position, sprinklerName, offsetIndex)
        local tool = getSprinklerTool(sprinklerName)
        if not tool then return false end
        
        local plotId = player:GetAttribute("PlotId")
        if not plotId then unequipTools(); return false end
        
        local finalPos = getOffsetPosition(position, offsetIndex)
        
        local rayParams = RaycastParams.new()
        rayParams.FilterType = Enum.RaycastFilterType.Include
        rayParams.FilterDescendantsInstances = workspace.Gardens:QueryDescendants("BasePart.PlantArea")
        
        local rayResult = workspace:Raycast(finalPos + Vector3.new(0, 10, 0), Vector3.new(0, -20, 0), rayParams)
        if rayResult then finalPos = rayResult.Position end
        
        local success = false
        pcall(function()
            networkingTools.Place.PlaceSprinkler:Fire(finalPos, sprinklerName, tool, plotId)
            success = true
        end)
        ...
        if success then
            ...
            sessionPlacedCount = sessionPlacedCount + 1
        end
        
        return success
    end
    
    local function useWateringCan(waterCanName)
        local tool = getWateringCanTool(waterCanName)
        if not tool then return false end
        
        local targetPos = nil
        if savedPosition then targetPos = savedPosition
        elseif selectedPlant then targetPos = selectedPlant.model:GetPivot().Position end
        
        if targetPos then
            pcall(function() networkingTools.WateringCan.UseWateringCan:Fire(targetPos, waterCanName, tool) end)
            return true
        end
        return false
    end
    
    local function doPlacementOnce()
        local selectedList = {}
        for name, selected in pairs(selectedSprinklers) do
            if selected then
                local amount = sprinklerAmounts[name] or 1
                if getSprinklerStock(name) <= 0 then return false, "❌ " .. name .. " habis!" end
                table.insert(selectedList, {name = name, amount = amount})
            end
        end
        if #selectedList == 0 then return false, "❌ Pilih sprinkler!" end
        
        local basePos = nil
        if selectedPlant then basePos = getPlantGroundPosition(selectedPlant.model)
        elseif savedPosition then basePos = savedPosition
        else return false, "❌ Simpan posisi / pilih plant!" end
        
        local totalPlaced = 0
        for _, sprinklerData in ipairs(selectedList) do
            for i = 1, sprinklerData.amount do
                if getSprinklerStock(sprinklerData.name) <= 0 then break end
                local ok = placeSprinklerAt(basePos, sprinklerData.name, placedCount + 1)
                if ok then totalPlaced = totalPlaced + 1; task.wait(0.5) else break end
            end
        end
        
        sprinklerBatchDone = true
        
        if totalPlaced == 0 then return false, "❌ Gagal place" end
        return true, "✅ " .. totalPlaced .. " sprinkler"
    end
    ...
    local function scanTrowelPlants()
        local plot = findMyPlot()
        if not plot then return {} end
        local plantsFolder = plot:FindFirstChild("Plants")
        if not plantsFolder then return {} end
        ...
        for _, plantModel in ipairs(plantsFolder:GetChildren()) do
            if plantModel:IsA("Model") then
                local seedName = plantModel:GetAttribute("SeedName")
                if selectedTrowelPlants[seedName] then
                    table.insert(plants, {name = seedName, model = plantModel, plantId = plantModel.Name})
                end
            end
        end
        return plants
    ...
    local function trowelPlant(plantModel, plantId, targetPos)
        local success = false
        pcall(function()
            networkingTools.Trowel.MovePlant:Fire(plantId, targetPos, 0)
            success = true
        end)
        if success then
            pcall(function() plantModel:PivotTo(CFrame.new(targetPos.X, targetPos.Y + 3, targetPos.Z)) end)
        end
        return success
    end
...
local function loadSelectedPlantFromConfig()
    ...
    toolsHeader.Size = UDim2.new(1, -12, 0, 20)
    toolsHeader.LayoutOrder = 1
    toolsHeader.Text = "🔧  Tools (Sprinkler + Water + Trowel)"
    toolsHeader.TextColor3 = C.text
    ...
    toolsHeader.Parent = toolsScroll
    
    -- Status
    local toolsStatus = Instance.new("TextLabel")
    toolsStatus.Size = UDim2.new(1, -12, 0, 14)
    toolsStatus.LayoutOrder = 2
    toolsStatus.Text = "✅ SIAP"
    toolsStatus.TextColor3 = C.green
    toolsStatus.Font = Enum.Font.GothamBold
    toolsStatus.TextSize = 9
    toolsStatus.TextXAlignment = Enum.TextXAlignment.Left
    toolsStatus.BackgroundTransparency = 1
    toolsStatus.Parent = toolsScroll
    
    -- Save Position Button
    local savePosBtn = Instance.new("TextButton")
    savePosBtn.Size = UDim2.new(1, -12, 0, 24)
    savePosBtn.LayoutOrder = 3
    savePosBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 200)
    savePosBtn.Text = "💾 SIMPAN POSISI"
    ...
    scanBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 180)
    scanBtn.Text = "🔍 SCAN PLANTS"
    scanBtn.TextColor3 = C.text
    scanBtn.Font = Enum.Font.GothamBold
    scanBtn.TextSize = 9
    scanBtn.BorderSizePixel = 0
    scanBtn.AutoButtonColor = false
    scanBtn.Parent = toolsScroll
    Instance.new("UICorner", scanBtn).CornerRadius = UDim.new(0, 4)

    -- Plant Dropdown (untuk memilih plant hasil scan)
    local plantDropdown = Instance.new("Frame")
    plantDropdown.Size = UDim2.new(1, -12, 0, 26)
    plantDropdown.LayoutOrder = 5
    plantDropdown.BackgroundColor3 = Color3.fromRGB(30, 30, 36)
    plantDropdown.BorderSizePixel = 0
    plantDropdown.Parent = toolsScroll
    Instance.new("UICorner", plantDropdown).CornerRadius = UDim.new(0, 4)

    local plantLabel = Instance.new("TextLabel")
    plantLabel.Size = UDim2.new(1, -30, 1, 0)
    plantLabel.Position = UDim2.new(0, 8, 0, 0)
    ...
    plantLabel.TextSize = 10
    plantLabel.TextXAlignment = Enum.TextXAlignment.Left
    plantLabel.Parent = plantDropdown

    local plantList = Instance.new("ScrollingFrame")
    plantList.Size = UDim2.new(1, 0, 0, 100)
    plantList.Position = UDim2.new(0, 0, 1, 3)
    plantList.BackgroundColor3 = Color3.fromRGB(35, 35, 40)
    plantList.BorderSizePixel = 0
    plantList.Visible = false
    plantList.ZIndex = 10
    plantList.Parent = plantDropdown
    Instance.new("UICorner", plantList).CornerRadius = UDim.new(0, 4)
    ...
    local plantClickArea = Instance.new("TextButton")
    plantClickArea.Size = UDim2.new(1, 0, 1, 0)
    plantClickArea.BackgroundTransparency = 1
    plantClickArea.Text = ""
    plantClickArea.ZIndex = 1
    plantClickArea.Parent = plantDropdown
    plantClickArea.MouseButton1Click:Connect(function()
        if #plantPositions == 0 then 
            plantLabel.Text = "Scan dulu!"; 
            return 
        end
        plantList.Visible = not plantList.Visible
    end)

    local function refreshPlantDropdown()
    -- Hapus semua opsi lama
        for _, child in ipairs(plantList:GetChildren()) do
            if child:IsA("TextButton") then child:Destroy() end
        end
    
        plantList.CanvasSize = UDim2.new(0, 0, 0, #plantPositions * 23 + 4)
    
        for _, plant in ipairs(plantPositions) do
            local opt = Instance.new("TextButton", plantList)
    ...
    -- Sprinkler Section Header
    local sprinklerHeader = Instance.new("TextLabel")
    sprinklerHeader.Size = UDim2.new(1, -12, 0, 12)
    sprinklerHeader.LayoutOrder = 6
    sprinklerHeader.Text = "💦 Sprinkler:"
    sprinklerHeader.TextColor3 = Color3.fromRGB(170, 170, 180)
    sprinklerHeader.Font = Enum.Font.Gotham
    sprinklerHeader.TextSize = 9
    sprinklerHeader.TextXAlignment = Enum.TextXAlignment.Left
    sprinklerHeader.BackgroundTransparency = 1
    sprinklerHeader.Parent = toolsScroll
    
    -- Sprinkler List
    local sprinklerListFrame = Instance.new("ScrollingFrame")
    sprinklerListFrame.Size = UDim2.new(1, -12, 0, 80)
    sprinklerListFrame.LayoutOrder = 7
    sprinklerListFrame.BackgroundColor3 = Color3.fromRGB(24, 24, 30)
    sprinklerListFrame.BorderSizePixel = 0
    sprinklerListFrame.ScrollBarThickness = 3
    sprinklerListFrame.ScrollBarImageColor3 = Color3.fromRGB(70, 70, 80)
    sprinklerListFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
    sprinklerListFrame.Parent = toolsScroll
    Instance.new("UICorner", sprinklerListFrame).CornerRadius = UDim.new(0, 4)
    
    local sprinklerListLayout = Instance.new("UIListLayout")
    sprinklerListLayout.Padding = UDim.new(0, 1)
    sprinklerListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    sprinklerListLayout.Parent = sprinklerListFrame
    
    -- Place Button
    local placeBtn = Instance.new("TextButton")
    placeBtn.Size = UDim2.new(0.48, 0, 0, 24)
    placeBtn.LayoutOrder = 8
    placeBtn.BackgroundColor3 = Color3.fromRGB(0, 160, 100)
    placeBtn.Text = "📌 PLACE"
    placeBtn.TextColor3 = C.text
    placeBtn.Font = Enum.Font.GothamBold
    placeBtn.TextSize = 9
    placeBtn.BorderSizePixel = 0
    placeBtn.AutoButtonColor = false
    placeBtn.Parent = toolsScroll
    Instance.new("UICorner", placeBtn).CornerRadius = UDim.new(0, 3)
    
    -- Auto Sprinkler Button
    local autoSprinklerBtn = Instance.new("TextButton")
    autoSprinklerBtn.Size = UDim2.new(0.48, 0, 0, 24)
    autoSprinklerBtn.LayoutOrder = 9
    autoSprinklerBtn.BackgroundColor3 = Color3.fromRGB(120, 60, 160)
    autoSprinklerBtn.Text = "🔄 AUTO"
    autoSprinklerBtn.TextColor3 = C.text
    autoSprinklerBtn.Font = Enum.Font.GothamBold
    autoSprinklerBtn.TextSize = 9
    autoSprinklerBtn.BorderSizePixel = 0
    autoSprinklerBtn.AutoButtonColor = false
    autoSprinklerBtn.Parent = toolsScroll
    Instance.new("UICorner", autoSprinklerBtn).CornerRadius = UDim.new(0, 3)
    
    -- Stop Sprinkler Button
    local stopSprinklerBtn = Instance.new("TextButton")
    stopSprinklerBtn.Size = UDim2.new(0.48, 0, 0, 24)
    stopSprinklerBtn.LayoutOrder = 10
    stopSprinklerBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
    stopSprinklerBtn.Text = "⏸ STOP"
    stopSprinklerBtn.TextColor3 = C.text
    stopSprinklerBtn.Font = Enum.Font.GothamBold
    stopSprinklerBtn.TextSize = 9
    stopSprinklerBtn.BorderSizePixel = 0
    stopSprinklerBtn.AutoButtonColor = false
    stopSprinklerBtn.Visible = false
    stopSprinklerBtn.Parent = toolsScroll
    Instance.new("UICorner", stopSprinklerBtn).CornerRadius = UDim.new(0, 3)
    
    -- Separator
    local sepWater = Instance.new("Frame")
    sepWater.Size = UDim2.new(1, -12, 0, 1)
    sepWater.LayoutOrder = 11
    sepWater.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
    sepWater.BorderSizePixel = 0
    sepWater.Parent = toolsScroll
    
    -- Watering Header
    local waterTitle = Instance.new("TextLabel")
    waterTitle.Size = UDim2.new(1, -12, 0, 14)
    waterTitle.LayoutOrder = 12
    waterTitle.Text = "🚿 WATERING"
    waterTitle.TextColor3 = Color3.fromRGB(0, 200, 255)
    waterTitle.Font = Enum.Font.GothamBold
    waterTitle.TextSize = 10
    waterTitle.TextXAlignment = Enum.TextXAlignment.Left
    waterTitle.BackgroundTransparency = 1
    waterTitle.Parent = toolsScroll
    
    -- Watering Can Dropdown
    local waterDropdown = Instance.new("Frame")
    waterDropdown.Size = UDim2.new(1, -12, 0, 24)
    waterDropdown.LayoutOrder = 13
    waterDropdown.BackgroundColor3 = Color3.fromRGB(30, 30, 36)
    waterDropdown.BorderSizePixel = 0
    waterDropdown.Parent = toolsScroll
    Instance.new("UICorner", waterDropdown).CornerRadius = UDim.new(0, 3)
    
    local waterLabelBtn = Instance.new("TextLabel")
    waterLabelBtn.Size = UDim2.new(1, -30, 1, 0)
    waterLabelBtn.Position = UDim2.new(0, 8, 0, 0)
    waterLabelBtn.BackgroundTransparency = 1
    waterLabelBtn.Text = selectedWateringCan ~= "" and selectedWateringCan or "Pilih..."
    waterLabelBtn.TextColor3 = selectedWateringCan ~= "" and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(180, 180, 180)
    waterLabelBtn.Font = Enum.Font.GothamBold
    waterLabelBtn.TextSize = 10
    waterLabelBtn.TextXAlignment = Enum.TextXAlignment.Left
    waterLabelBtn.Parent = waterDropdown
    
    local waterList = Instance.new("ScrollingFrame")
    waterList.Size = UDim2.new(1, 0, 0, 70)
    waterList.Position = UDim2.new(0, 0, 1, 3)
    waterList.BackgroundColor3 = Color3.fromRGB(35, 35, 40)
    waterList.BorderSizePixel = 0
    waterList.Visible = false
    waterList.ZIndex = 10
    waterList.Parent = waterDropdown
    Instance.new("UICorner", waterList).CornerRadius = UDim.new(0, 3)
    
    local waterListLayout = Instance.new("UIListLayout")
    waterListLayout.Padding = UDim.new(0, 1)
    waterListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    waterListLayout.Parent = waterList
    
    local waterClickArea = Instance.new("TextButton")
    waterClickArea.Size = UDim2.new(1, 0, 1, 0)
    waterClickArea.BackgroundTransparency = 1
    waterClickArea.Text = ""
    waterClickArea.ZIndex = 1
    ...
    intervalInput.Size = UDim2.new(0.48, 0, 0, 22)
    intervalInput.LayoutOrder = 14
    intervalInput.BackgroundColor3 = Color3.fromRGB(30, 30, 36)
    intervalInput.TextColor3 = Color3.fromRGB(255, 255, 255)
    intervalInput.PlaceholderText = "Interval (30)"
    intervalInput.PlaceholderColor3 = Color3.fromRGB(100, 100, 110)
    intervalInput.Font = Enum.Font.Gotham
    intervalInput.TextSize = 9
    intervalInput.Text = tostring(wateringInterval)
    intervalInput.Parent = toolsScroll
    Instance.new("UICorner", intervalInput).CornerRadius = UDim.new(0, 3)
    
    -- Auto Water Button
    local autoWaterBtn = Instance.new("TextButton")
    autoWaterBtn.Size = UDim2.new(0.48, 0, 0, 22)
    autoWaterBtn.LayoutOrder = 15
    autoWaterBtn.BackgroundColor3 = Color3.fromRGB(0, 180, 220)
    autoWaterBtn.Text = "🚿 AUTO WATER"
    autoWaterBtn.TextColor3 = C.text
    autoWaterBtn.Font = Enum.Font.GothamBold
    autoWaterBtn.TextSize = 9
    autoWaterBtn.BorderSizePixel = 0
    autoWaterBtn.AutoButtonColor = false
    autoWaterBtn.Parent = toolsScroll
    Instance.new("UICorner", autoWaterBtn).CornerRadius = UDim.new(0, 3)
    
    -- Separator Trowel
    local sepTrowel = Instance.new("Frame")
    sepTrowel.Size = UDim2.new(1, -12, 0, 1)
    sepTrowel.LayoutOrder = 16
    sepTrowel.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
    sepTrowel.BorderSizePixel = 0
    sepTrowel.Parent = toolsScroll
    
    -- Trowel Header
    local trowelTitle = Instance.new("TextLabel")
    trowelTitle.Size = UDim2.new(1, -12, 0, 14)
    trowelTitle.LayoutOrder = 17
    trowelTitle.Text = "🔧 TROWEL"
    trowelTitle.TextColor3 = Color3.fromRGB(200, 150, 0)
    trowelTitle.Font = Enum.Font.GothamBold
    trowelTitle.TextSize = 10
    trowelTitle.TextXAlignment = Enum.TextXAlignment.Left
    trowelTitle.BackgroundTransparency = 1
    trowelTitle.Parent = toolsScroll
    
    -- Trowel Save Button
    local trowelSaveBtn = Instance.new("TextButton")
    trowelSaveBtn.Size = UDim2.new(0.48, 0, 0, 22)
    trowelSaveBtn.LayoutOrder = 18
    trowelSaveBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 200)
    trowelSaveBtn.Text = "💾 SAVE TARGET"
    trowelSaveBtn.TextColor3 = C.text
    trowelSaveBtn.Font = Enum.Font.GothamBold
    trowelSaveBtn.TextSize = 9
    trowelSaveBtn.BorderSizePixel = 0
    trowelSaveBtn.AutoButtonColor = false
    trowelSaveBtn.Parent = toolsScroll
    Instance.new("UICorner", trowelSaveBtn).CornerRadius = UDim.new(0, 3)
    
    -- Trowel Interval
    local trowelIntervalInput = Instance.new("TextBox")
    trowelIntervalInput.Size = UDim2.new(0.48, 0, 0, 22)
    trowelIntervalInput.LayoutOrder = 19
    trowelIntervalInput.BackgroundColor3 = Color3.fromRGB(30, 30, 36)
    trowelIntervalInput.TextColor3 = Color3.fromRGB(255, 255, 255)
    trowelIntervalInput.PlaceholderText = "Interval (1.0)"
    trowelIntervalInput.PlaceholderColor3 = Color3.fromRGB(100, 100, 110)
    trowelIntervalInput.Font = Enum.Font.Gotham
    trowelIntervalInput.TextSize = 9
    trowelIntervalInput.Text = tostring(trowelInterval)
    trowelIntervalInput.Parent = toolsScroll
    Instance.new("UICorner", trowelIntervalInput).CornerRadius = UDim.new(0, 3)
    
    -- Trowel Search
    local trowelSearch = Instance.new("TextBox")
    trowelSearch.Size = UDim2.new(1, -12, 0, 20)
    trowelSearch.LayoutOrder = 20
    trowelSearch.BackgroundColor3 = Color3.fromRGB(30, 30, 36)
    trowelSearch.TextColor3 = Color3.fromRGB(255, 255, 255)
    trowelSearch.PlaceholderText = "🔍 Cari plant..."
    trowelSearch.PlaceholderColor3 = Color3.fromRGB(100, 100, 110)
    trowelSearch.Font = Enum.Font.Gotham
    trowelSearch.TextSize = 9
    trowelSearch.Text = ""
    trowelSearch.ClearTextOnFocus = false
    trowelSearch.Parent = toolsScroll
    Instance.new("UICorner", trowelSearch).CornerRadius = UDim.new(0, 3)
    
    -- Trowel List
    local trowelListFrame = Instance.new("ScrollingFrame")
    trowelListFrame.Size = UDim2.new(1, -12, 0, 80)
    trowelListFrame.LayoutOrder = 21
    trowelListFrame.BackgroundColor3 = Color3.fromRGB(24, 24, 30)
    trowelListFrame.BorderSizePixel = 0
    trowelListFrame.ScrollBarThickness = 3
    trowelListFrame.ScrollBarImageColor3 = Color3.fromRGB(70, 70, 80)
    trowelListFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
    trowelListFrame.Parent = toolsScroll
    Instance.new("UICorner", trowelListFrame).CornerRadius = UDim.new(0, 3)
    
    local trowelListLayout = Instance.new("UIListLayout")
    trowelListLayout.Padding = UDim.new(0, 1)
    trowelListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    trowelListLayout.Parent = trowelListFrame
    
    -- Trowel Start Button
    local trowelStartBtn = Instance.new("TextButton")
    trowelStartBtn.Size = UDim2.new(0.48, 0, 0, 24)
    trowelStartBtn.LayoutOrder = 22
    trowelStartBtn.BackgroundColor3 = Color3.fromRGB(200, 120, 0)
    trowelStartBtn.Text = "🔧 MULAI TROWEL"
    trowelStartBtn.TextColor3 = C.text
    trowelStartBtn.Font = Enum.Font.GothamBold
    trowelStartBtn.TextSize = 9
    trowelStartBtn.BorderSizePixel = 0
    trowelStartBtn.AutoButtonColor = false
    trowelStartBtn.Parent = toolsScroll
    Instance.new("UICorner", trowelStartBtn).CornerRadius = UDim.new(0, 3)
    
    -- Trowel Stop Button
    local trowelStopBtn = Instance.new("TextButton")
    trowelStopBtn.Size = UDim2.new(0.48, 0, 0, 24)
    trowelStopBtn.LayoutOrder = 23
    trowelStopBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
    trowelStopBtn.Text = "⏸ STOP"
    trowelStopBtn.TextColor3 = C.text
    trowelStopBtn.Font = Enum.Font.GothamBold
    trowelStopBtn.TextSize = 9
    trowelStopBtn.BorderSizePixel = 0
    trowelStopBtn.AutoButtonColor = false
    trowelStopBtn.Visible = false
    trowelStopBtn.Parent = toolsScroll
    Instance.new("UICorner", trowelStopBtn).CornerRadius = UDim.new(0, 3)
    
    -- ============================================
    -- REFRESH FUNCTIONS
    -- ============================================
    local function refreshSprinklerList()
        for _, child in ipairs(sprinklerListFrame:GetChildren()) do
            if child:IsA("Frame") then child:Destroy() end
        end
        
        local totalRows = 0
        for _, name in ipairs(sprinklerNames) do
            local stock = getSprinklerStock(name)
            local isSelected = selectedSprinklers[name] == true
            local amount = sprinklerAmounts[name] or 1
            
            local row = Instance.new("Frame", sprinklerListFrame)
            row.Size = UDim2.new(1, -8, 0, 22)
            row.BackgroundColor3 = isSelected and Color3.fromRGB(40, 60, 45) or (stock > 0 and Color3.fromRGB(38, 38, 44) or Color3.fromRGB(30, 25, 25))
            row.BorderSizePixel = 0
            Instance.new("UICorner", row).CornerRadius = UDim.new(0, 2)
            ...
            local lb = Instance.new("TextLabel", row)
            lb.Size = UDim2.new(1, -100, 1, 0); lb.Position = UDim2.new(0, 18, 0, 0)
            lb.BackgroundTransparency = 1; lb.Text = name
            lb.TextColor3 = stock > 0 and Color3.fromRGB(200, 200, 200) or Color3.fromRGB(120, 100, 100)
            lb.Font = Enum.Font.Gotham; lb.TextSize = 8; lb.TextXAlignment = Enum.TextXAlignment.Left
            
            local amountInput = Instance.new("TextBox", row)
            amountInput.Size = UDim2.new(0, 25, 0, 16)
            amountInput.Position = UDim2.new(1, -58, 0.5, -8)
            amountInput.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
            amountInput.TextColor3 = C.text
            amountInput.Font = Enum.Font.Gotham
            amountInput.TextSize = 8
            amountInput.Text = tostring(amount)
            amountInput.PlaceholderText = "1"
            amountInput.PlaceholderColor3 = Color3.fromRGB(100, 100, 100)
            Instance.new("UICorner", amountInput).CornerRadius = UDim.new(0, 2)
            
            amountInput.FocusLost:Connect(function()
                local val = tonumber(amountInput.Text)
                if val and val > 0 then 
                    sprinklerAmounts[name] = val
                    config.toolsSprinklerAmounts[name] = val
                    saveConfig()
                else 
                    amountInput.Text = tostring(sprinklerAmounts[name] or 1) 
                end
            end)
            
            local stockLabel = Instance.new("TextLabel", row)
            stockLabel.Size = UDim2.new(0, 25, 1, 0); stockLabel.Position = UDim2.new(1, -30, 0, 0)
            stockLabel.BackgroundTransparency = 1
            stockLabel.Text = "x" .. stock
            stockLabel.TextColor3 = stock > 0 and Color3.fromRGB(0, 255, 150) or Color3.fromRGB(150, 80, 80)
            stockLabel.Font = Enum.Font.GothamBold; stockLabel.TextSize = 7
            stockLabel.TextXAlignment = Enum.TextXAlignment.Right
            
            local function toggle()
                if stock <= 0 then return end
                selectedSprinklers[name] = not selectedSprinklers[name]
                config.toolsSprinklerSelected[name] = selectedSprinklers[name]
                saveConfig()
        ...
        sprinklerListFrame.CanvasSize = UDim2.new(0, 0, 0, totalRows * 23 + 4)
    end
    
    local function refreshTrowelList()
        for _, child in ipairs(trowelListFrame:GetChildren()) do
            if child:IsA("Frame") then child:Destroy() end
        end
        ...
        for _, name in ipairs(allSeedNames) do table.insert(sorted, name) end
        table.sort(sorted, function(a, b)
            local aSel = selectedTrowelPlants[a] and 1 or 0
            local bSel = selectedTrowelPlants[b] and 1 or 0
            if aSel ~= bSel then return aSel > bSel end
            return a < b
        end)
        ...
        local filtered = {}
        for _, name in ipairs(sorted) do
            if trowelSearchText == "" or name:lower():find(trowelSearchText) then table.insert(filtered, name) end
        end
        
        trowelListFrame.CanvasSize = UDim2.new(0, 0, 0, #filtered * 19 + 4)
        
        for _, name in ipairs(filtered) do
            local isSelected = selectedTrowelPlants[name] == true
            local row = Instance.new("Frame", trowelListFrame)
            row.Size = UDim2.new(1, -8, 0, 17)
            row.BackgroundColor3 = isSelected and Color3.fromRGB(40, 60, 45) or Color3.fromRGB(38, 38, 44)
            row.BorderSizePixel = 0
            Instance.new("UICorner", row).CornerRadius = UDim.new(0, 2)
            ...
            cb.BackgroundColor3 = isSelected and Color3.fromRGB(0, 160, 80) or Color3.fromRGB(55, 55, 60)
            cb.Text = isSelected and "✓" or ""; cb.TextColor3 = C.text
            cb.Font = Enum.Font.GothamBold; cb.TextSize = 9; cb.BorderSizePixel = 0
            Instance.new("UICorner", cb).CornerRadius = UDim.new(0, 2)
            
            local lb = Instance.new("TextLabel", row)
            lb.Size = UDim2.new(1, -18, 1, 0); lb.Position = UDim2.new(0, 16, 0, 0)
            lb.BackgroundTransparency = 1; lb.Text = name
            lb.TextColor3 = isSelected and Color3.fromRGB(200, 255, 200) or Color3.fromRGB(200, 200, 200)
            lb.Font = Enum.Font.Gotham; lb.TextSize = 8; lb.TextXAlignment = Enum.TextXAlignment.Left
            
            local function toggle()
                selectedTrowelPlants[name] = not selectedTrowelPlants[name]
                config.toolsTrowelSelected[name] = selectedTrowelPlants[name]
                saveConfig()
                refreshTrowelList()
            end
            cb.MouseButton1Click:Connect(toggle)
    ...
    placeBtn.MouseButton1Click:Connect(function()
        ...
        isPlacing = true
        toolsStatus.Text = "📌 Placing..."; toolsStatus.TextColor3 = C.yellow
        task.spawn(function()
            local ok, msg = doPlacementOnce()
            toolsStatus.Text = msg
            toolsStatus.TextColor3 = ok and C.green or C.red
            isPlacing = false
            refreshSprinklerList()
        end)
    end)
    
    autoSprinklerBtn.MouseButton1Click:Connect(function()
        if autoRunning then return end
        autoRunning = true
        config.toolsIsAutoSprinkler = true
        saveConfig()
        sessionActive = false
        sessionPlacedCount = 0
        sessionPositions = {}
        placedCount = 0
        sprinklerBatchDone = false
        placeBtn.Visible = false
        autoSprinklerBtn.Visible = false
        stopSprinklerBtn.Visible = true
        toolsStatus.Text = "🔄 Auto..."; toolsStatus.TextColor3 = C.green
        
        task.spawn(function()
            while autoRunning do
                if sessionActive then
                    local elapsed = tick() - sessionStartTime
                    if elapsed >= sessionTimer then
                        sessionActive = false
                        sessionPlacedCount = 0
                        sessionPositions = {}
                        placedCount = 0
                    ...
                    toolsStatus.Text = msg
                    toolsStatus.TextColor3 = ok and C.green or C.red
                    if not ok then autoRunning = false; break end
                end
                task.wait(1)
            end
            placeBtn.Visible = true
            autoSprinklerBtn.Visible = true
            stopSprinklerBtn.Visible = false
            toolsStatus.Text = "⏸ Berhenti"
    ...
    trowelSaveBtn.MouseButton1Click:Connect(function()
        local pos = getPlayerPosition()
        if not pos then toolsStatus.Text = "❌ Gagal!"; return end
        local rayParams = RaycastParams.new()
        rayParams.FilterType = Enum.RaycastFilterType.Include
        rayParams.FilterDescendantsInstances = workspace.Gardens:QueryDescendants("BasePart.PlantArea")
        local rayResult = workspace:Raycast(pos + Vector3.new(0, 50, 0), Vector3.new(0, -100, 0), rayParams)
        if rayResult then trowelTargetPosition = rayResult.Position else trowelTargetPosition = pos end
        toolsStatus.Text = "✅ Target trowel disimpan!"
        toolsStatus.TextColor3 = C.green
    end)
    
    trowelStartBtn.MouseButton1Click:Connect(function()
        if isTrowelRunning then return end
        
        if not trowelTargetPosition then toolsStatus.Text = "❌ Save target dulu!"; return end
        if not hasSelection(selectedTrowelPlants) then toolsStatus.Text = "❌ Pilih plant!"; return end
        
        local trowel = getTrowelTool()
        if not trowel then toolsStatus.Text = "❌ Trowel tidak ditemukan!"; return end
        
        isTrowelRunning = true
        config.toolsIsTrowelRunning = true
        saveConfig()
        trowelStartBtn.Visible = false
        trowelStopBtn.Visible = true
        
        task.spawn(function()
            task.wait(0.5)
            toolsStatus.Text = "🔍 Scanning..."; toolsStatus.TextColor3 = C.yellow
            local plants = scanTrowelPlants()
            
            if #plants == 0 then
                toolsStatus.Text = "❌ Tidak ada plant"
                toolsStatus.TextColor3 = C.red
            else
                trowelInterval = tonumber(trowelIntervalInput.Text) or 1.0
                config.toolsTrowelInterval = trowelInterval
                saveConfig()
                local moved = 0
                for i, plant in ipairs(plants) do
                    if not isTrowelRunning then break end
                    toolsStatus.Text = "🔧 #" .. i .. "/" .. #plants
                    trowelPlant(plant.model, plant.plantId, trowelTargetPosition)
                    moved = moved + 1
                    if i < #plants and isTrowelRunning then task.wait(trowelInterval) end
                end
                toolsStatus.Text = "✅ " .. moved .. " plant dipindah"
                toolsStatus.TextColor3 = C.green
            end
            
            isTrowelRunning = false
            config.toolsIsTrowelRunning = false
            saveConfig()
            trowelStartBtn.Visible = true
            trowelStopBtn.Visible = false
        end)
    end)
    
    trowelStopBtn.MouseButton1Click:Connect(function() 
        isTrowelRunning = false 
    end)
    
    trowelSearch:GetPropertyChangedSignal("Text"):Connect(function()
        trowelSearchText = trowelSearch.Text:lower()
        refreshTrowelList()
    end)
    ...
    intervalInput.FocusLost:Connect(function()
        ...
        if val and val > 0 then
            ...
            config.toolsWateringInterval = val
            saveConfig()
        else
            intervalInput.Text = tostring(wateringInterval)
        end
    ...
    trowelIntervalInput.FocusLost:Connect(function()
        local val = tonumber(trowelIntervalInput.Text)
        if val and val > 0 then
            trowelInterval = val
            config.toolsTrowelInterval = val
            saveConfig()
        else
            trowelIntervalInput.Text = tostring(trowelInterval)
        end
...
-- ==================================================================
-- VALUE DISPLAY SYSTEM (UPDATED - DENGAN AUTO-REPAIR & HOTBAR REFRESH)
-- ==================================================================
do
    ...
    -- Create label for slot (DENGAN AUTO-REPAIR)
    local function CreateLabelForSlot(slot)
        ...
        -- Auto-repair jika label dihancurkan
        sellValueLabel.Destroying:Connect(function()
            if not config.extraToggle1 then return end
            task.wait(0.2)
            pcall(function()
                if slot and slot.Parent then
                    local oldLabel = slot:FindFirstChild("SellValue")
                    if not oldLabel then
                        CreateLabelForSlot(slot)
                    end
                end
            end)
        end)
    ...
    -- Force update label (untuk hotbar refresh)
    local function ForceUpdateLabelForSlot(slot)
        if not config.extraToggle1 then return end
        if not slot or not slot:IsA("TextButton") then return end
        ...
        -- Jika tidak ada ToolName atau kosong, hapus label
        if not toolNameLabel or not toolNameLabel:IsA("TextLabel") or toolNameLabel.Text == "" then
            local existingLabel = slot:FindFirstChild("SellValue")
            if existingLabel then
                existingLabel.Destroying:Disconnect()
        ...
        -- Jika tidak ada fruit data, hapus label
        ...
        local sellValueLabel = slot:FindFirstChild("SellValue")
        if not sellValueLabel then
    ...
    -- Refresh hotbar slots (force update)
    local function RefreshHotbarSlots()
        ...
        for _, slot in hotbar:GetChildren() do
            if slot:IsA("TextButton") or slot:IsA("Frame") then
                pcall(function() ForceUpdateLabelForSlot(slot) end)
            end
        end
    end
    ...
    -- Update frame dan refresh hotbar secara berkala
    local function StartPeriodicUpdate()
        task.spawn(function()
            while true do
                task.wait(5)  -- Setiap 5 detik (sesuai script original)
...
if config.toolsIsAutoSprinkler then
    task.delay(7, function()
        print("[AoneHub] Tools: Auto Sprinkler was running, silakan restart manual")
    end)
  end
